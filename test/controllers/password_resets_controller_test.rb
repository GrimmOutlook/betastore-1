...
class PasswordResetsControllerTest < ActionController::TestCase
	...
	test "edit with vaild id and token" do
	...
	test "edit with invalid id and invalid token" do
		get :edit, id: 0, token: 'fail'
		assert_redirected_to forgot_password_path
	end

	test "update with vaild id and token the password changes" do
		assert customers(:test).authenticate('password')
		post :update, id: password_resets(:test).id,
				   token: password_resets(:test).token,
				   password: 'secret'
		# The database has changed, but this object memory has not
